...
// PID constants for different turn angles
...
/**
 * Standard WP autonomous routine.
 * Sequence: reload → boost → intake + toggle → drive + turn maneuvers → multiple shots.
 */
void wp() 
{
    // s = r*theta, r = s/theta
...
/**
 * Skills run (expansion + long-cycle scoring).
 */
void skills()
{
    intake::spin(127);
...
// Autonomous routine registration
fptr WP = wp; 
