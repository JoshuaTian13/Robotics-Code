...
/**
 * Button mappings for controller input.
 * Each enum entry represents a press/release state.
 */
enum buttons {
    L1 = 0,  NL1 = 1,
    L2 = 2,  NL2 = 3,
...
/**
 * Control scheme: KEEJ
 * Arcade drive + catapult + intake + expansion + boost toggles.
 */
void keej() {
...
/**
 * Control scheme: FELIX
 * Alternate drive direction + similar subsystem mapping.
 */
void felix() {
    std::vector<bool> cont = robot::controller.getAll(ALLBUTTONS);
    chass.spinDiffy(robot::controller.drive(-1, util::controller::arcade));
